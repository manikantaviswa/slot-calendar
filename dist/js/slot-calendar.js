/**
 * slot-calendar - A pure AngularJS bootstrap themed responsive calendar that can display events and has views for year, month, week and day
 * @version v1.0.0
 * @link https://github.com/mattlewis92/slot-calendar
 * @license MIT
 */
...
angular
  .module('mwl.calendar')
  .controller('MwlCalendarCtrl', ["$scope", "$log", "$timeout", "$attrs", "$locale", "abcMoment", "calendarTitle", "calendarHelper", function($scope, $log, $timeout, $attrs, $locale, abcMoment, calendarTitle, calendarHelper) {
    ...
    vm.dateClicked = function(date) {
      ...
      var rawDate = abcMoment(date).toDate();
    ...
    vm.$onInit = function() {
      ...
      var previousDate = abcMoment(vm.viewDate);
      ...
      function checkEventIsValid(event) {
        ...
        if (event.endsAt) {
          ...
          if (abcMoment(event.startsAt).isAfter(abcMoment(event.endsAt))) {
      ...
      function refreshCalendar() {
        ...
        var currentDate = abcMoment(vm.viewDate);
      ...
      calendarHelper.loadTemplates().then(function() {
        ...
        $scope.$watchGroup([
          ...
          'vm.cellIsOpen',
          function() {
            return abcMoment.locale() + $locale.id; //Auto update the calendar when the locale changes
          }
        ], function() {
          if (!eventsWatched) {
            eventsWatched = true;
...
angular
  .module('mwl.calendar')
  .controller('MwlCalendarDayCtrl', ["$scope", "abcMoment", "calendarHelper", "calendarEventTitle", function($scope, abcMoment, calendarHelper, calendarEventTitle) {
...
angular
  .module('mwl.calendar')
  .controller('MwlCalendarHourListCtrl', ["$scope", "$document", "abcMoment", "calendarHelper", function($scope, $document, abcMoment, calendarHelper) {
    var vm = this;
    ...
    function updateDays() {
      ...
      vm.hourGrid = calendarUtils.getDayViewHourGrid({
        viewDate: vm.view === 'week' ? abcMoment(vm.viewDate).startOf('week').toDate() : abcMoment(vm.viewDate).toDate(),
      ...
      vm.hourGrid.forEach(function(hour) {
        hour.segments.forEach(function(segment) {
          ...
          segment.date = abcMoment(segment.date);
          ...
          if (vm.view === 'week') {
            ...
            for (var i = 0; i < 7; i++) {
              var day = {
                date: abcMoment(segment.date).add(i, 'days')
              };
    ...
    var originalLocale = abcMoment.locale();

    $scope.$on('calendar.refreshView', function() {

      if (originalLocale !== abcMoment.locale()) {
        originalLocale = abcMoment.locale();
        updateDays();
      }
...
angular
  .module('mwl.calendar')
  .controller('MwlCalendarMonthCtrl', ["$scope", "abcMoment", "calendarHelper", "calendarConfig", "calendarEventTitle", function($scope, abcMoment, calendarHelper, calendarConfig, calendarEventTitle) {
    ...
    function toggleCell() {
      ...
      if (vm.cellIsOpen && vm.view && vm.weekDays) {
        vm.view.forEach(function(day, dayIndex) {
          if (abcMoment(vm.viewDate).startOf('day').isSame(day.date)) {
    ...
    $scope.$on('calendar.refreshView', function() {
      ...
      if (vm.cellAutoOpenDisabled) {
        ...
        vm.view.forEach(function(day) {
          if (day.inMonth && abcMoment(vm.viewDate).startOf('day').isSame(day.date)) {
    ...
    vm.handleEventDrop = function(event, newDayDate, draggedFromDate) {
      ...
      var newStart = abcMoment(event.startsAt)
        .year(abcMoment(newDayDate).year())
        .month(abcMoment(newDayDate).month())
        .date(abcMoment(newDayDate).date());
...
angular
  .module('mwl.calendar')
  .controller('MwlCalendarWeekCtrl', ["$scope", "abcMoment", "calendarHelper", "calendarConfig", "calendarEventTitle", function($scope, abcMoment, calendarHelper, calendarConfig, calendarEventTitle) {
    ...
    vm.weekDragged = function(event, daysDiff, minuteChunksMoved) {
      ...
      var newStart = abcMoment(event.startsAt).add(daysDiff, 'days');
      var newEnd = abcMoment(event.endsAt).add(daysDiff, 'days');
    ...
    vm.eventDropped = function(event, date) {
      var daysDiff = abcMoment(date).diff(abcMoment(event.startsAt), 'days');
...
angular
  .module('mwl.calendar')
  .controller('MwlCalendarYearCtrl', ["$scope", "abcMoment", "calendarHelper", function($scope, abcMoment, calendarHelper) {
    ...
    function toggleCell() {
      ...
      if (vm.cellIsOpen && vm.view) {
        vm.view.forEach(function(month, monthIndex) {
          if (abcMoment(vm.viewDate).startOf('month').isSame(month.date)) {
    ...
    vm.handleEventDrop = function(event, newMonthDate) {
      var newStart = abcMoment(event.startsAt)
        .year(abcMoment(newMonthDate).year())
        .month(abcMoment(newMonthDate).month());
      var newEnd = calendarHelper.adjustEndDateFromStartDiff(event.startsAt, newStart, event.endsAt);
...
angular
  .module('mwl.calendar')
  .controller('MwlDateModifierCtrl', ["$element", "$attrs", "$scope", "abcMoment", function($element, $attrs, $scope, abcMoment) {
    ...
    function onClick() {
      if (angular.isDefined($attrs.setToToday)) {
        ...
        vm.date = abcMoment(vm.date).add(1, vm.increment);
        ...
        vm.date = abcMoment(vm.date).subtract(1, vm.decrement);
...
angular
  .module('mwl.calendar')
  .factory('calendarHelper', ["$q", "$templateRequest", "dateFilter", "abcMoment", "calendarConfig", function($q, $templateRequest, dateFilter, abcMoment, calendarConfig) {

    function formatDate(date, format) {
      if (calendarConfig.dateFormatter === 'angular') {
        return dateFilter(abcMoment(date).toDate(), format);
      } else if (calendarConfig.dateFormatter === 'moment') {
        return abcMoment(date).format(format);
      } else {
    ...
    function adjustEndDateFromStartDiff(oldStart, newStart, oldEnd) {
      ...
      var diffInSeconds = abcMoment(newStart).diff(abcMoment(oldStart));
      return abcMoment(oldEnd).add(diffInSeconds);
    }

    function getRecurringEventPeriod(eventPeriod, recursOn, containerPeriodStart) {

      var eventStart = abcMoment(eventPeriod.start);
      var eventEnd = abcMoment(eventPeriod.end);
      var periodStart = abcMoment(containerPeriodStart);
    ...
    function eventIsInPeriod(event, periodStart, periodEnd) {
      ...
      periodStart = abcMoment(periodStart);
      periodEnd = abcMoment(periodEnd);
    ...
    function getEventsInPeriod(calendarDate, period, allEvents) {
      var startPeriod = abcMoment(calendarDate).startOf(period);
      var endPeriod = abcMoment(calendarDate).endOf(period);
    ...
    function getWeekDayNames(excluded) {
      ...
      .map(function(i) {
        return formatDate(abcMoment().weekday(i), calendarConfig.dateFormats.weekDay);
      });
    ...
    function getYearView(events, viewDate, cellModifier) {
      ...
      var month = abcMoment(viewDate).startOf('year');
      var count = 0;
      while (count < 12) {
        ...
        var cell = {
          ...
          isToday: startPeriod.isSame(abcMoment().startOf('month')),
          events: periodEvents,
          date: startPeriod,
    ...
    function getMonthView(events, viewDate, cellModifier, excluded) {
      ...
      events.forEach(function(event) {
        var eventPeriod = getRecurringEventPeriod({
          start: abcMoment(event.startsAt),
          end: abcMoment(event.endsAt || event.startsAt)
        }, event.recursOn, abcMoment(viewDate).startOf('month'));
      ...
      var view = calendarUtils.getMonthView({
        ...
        viewDate: viewDate,
        excluded: excluded,
        weekStartsOn: abcMoment().startOf('week').day()
      });

      view.days = view.days.map(function(day) {
        day.date = abcMoment(day.date);
    ...
    function getWeekView(events, viewDate, excluded) {
      ...
      var startOfWeek = abcMoment(viewDate).startOf('week');
      var endOfWeek = abcMoment(viewDate).endOf('week');
      ...
      var eventRows = calendarUtils.getWeekView({
        ...
        events: filterEventsInPeriod(events, startOfWeek, endOfWeek).map(function(event) {
          ...
          var weekViewStart = abcMoment(startOfWeek).startOf('day');
    ...
    function getDayView(events, viewDate, dayViewStart, dayViewEnd, dayViewSplit, dayViewEventWidth, dayViewSegmentSize) {
      ...
      var view = calendarUtils.getDayView({
        events: events.map(function(event) { // hack required to work with event API
          ...
          }, event.recursOn, abcMoment(viewDate).startOf('day'));
    ...
    function getWeekViewWithTimes(events, viewDate, dayViewStart, dayViewEnd, dayViewSplit) {
      ...
      weekView.days.forEach(function(day) {
        var dayEvents = flattenedEvents.filter(function(event) {
          return abcMoment(event.startsAt).startOf('day').isSame(abcMoment(day.date).startOf('day'));
        });
        ...
        row: newEvents.map(function(dayEvent) {
          ...
          return {
            ...
            offset: calendarUtils.getWeekViewEventOffset({
              ...
              startOfWeek: abcMoment(viewDate).startOf('week').toDate()
            })
          };
        })
      }];
    ...
    function getDayViewHeight(dayViewStart, dayViewEnd, dayViewSplit, dayViewSegmentSize) {
      var dayViewStartM = abcMoment(dayViewStart || '00:00', 'HH:mm');
      var dayViewEndM = abcMoment(dayViewEnd || '23:59', 'HH:mm');
...
angular
  .module('mwl.calendar')
  .factory('calendarTitle', ["abcMoment", "calendarConfig", "calendarHelper", function(abcMoment, calendarConfig, calendarHelper) {
